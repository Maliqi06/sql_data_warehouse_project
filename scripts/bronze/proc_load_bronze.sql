...
BEGIN
	...
	PRINT 'Inserting data into bronze.source_product...';
	BEGIN TRY
		TRUNCATE TABLE bronze.source_product;
		BULK INSERT bronze.source_product 
		FROM 'C:\data\source\product.csv' 
		WITH (
			FORMAT = 'CSV',
			FIRSTROW = 2,
			FIELDTERMINATOR = ',',
		...
		PRINT 'Successfully inserted data into bronze.source_product.';
	END TRY
	BEGIN CATCH
		PRINT 'Error inserting data into bronze.source_product: ' + ERROR_MESSAGE();
	END CATCH;
